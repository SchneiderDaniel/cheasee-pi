...
// ─── Tests: parseGitStatusPorcelain extracted helper ───────────────

describe("parseGitStatusPorcelain — extracted helper", () => {
	it("parses normal git status output → dirty files correctly parsed", async () => {
		const { parseGitStatusPorcelain } = await import("../../pipeline/handler.ts");
		const stdout = " M src/file1.ts\n?? untracked.txt\n";
		const files = parseGitStatusPorcelain(stdout);
		assert.deepEqual(files, ["src/file1.ts", "untracked.txt"]);
	});

	it("parses git status with renamed file → correctly resolves to target path", async () => {
		const { parseGitStatusPorcelain } = await import("../../pipeline/handler.ts");
		const stdout = "R  old/path.ts -> new/path.ts\n";
		const files = parseGitStatusPorcelain(stdout);
		assert.deepEqual(files, ["new/path.ts"]);
	});

	it("parses git status with empty output → returns empty array", async () => {
		const { parseGitStatusPorcelain } = await import("../../pipeline/handler.ts");
		const files = parseGitStatusPorcelain("");
		assert.deepEqual(files, []);
	});

	it("parses git status with 'R  old -> new' rename → maps to 'new' only", async () => {
		const { parseGitStatusPorcelain } = await import("../../pipeline/handler.ts");
		const stdout = "R  src/old.ts -> src/new.ts\n M src/other.ts\n";
		const files = parseGitStatusPorcelain(stdout);
		assert.deepEqual(files, ["src/new.ts", "src/other.ts"]);
	});

	it("handles multiple status types mixed", async () => {
		const { parseGitStatusPorcelain } = await import("../../pipeline/handler.ts");
		const stdout = " M modified.ts\n A added.ts\n D deleted.ts\n?? untracked.ts\n";
		const files = parseGitStatusPorcelain(stdout);
		assert.deepEqual(files, ["modified.ts", "added.ts", "deleted.ts", "untracked.ts"]);
	});
});
