...
func main() {
	...
	// Track recently-killed PIDs to avoid re-signaling zombies
	// that haven't been reaped yet. Reset each sweep cycle.
	recentlyKilled := make(map[int]bool)

	run := func() bool {
		// Clear last cycle's killed set — zombies that persist beyond
		// one interval will be re-detected and re-killed harmlessly,
		// but reaping normally clears them within one tick.
		for pid := range recentlyKilled {
			delete(recentlyKilled, pid)
		}
		...
		for _, pid := range pids {
			if recentlyKilled[pid] {
				continue
			}
			if escalate(pid, cfg.dryRun, logger) {
				recentlyKilled[pid] = true
			}
		}
		return true // continue
	}
...
// Returns list of PIDs where the process (or its bash-wrapper parent)
// is orphaned (PPid=1) and cmdline contains pidName.
//
// Two orphan patterns:
//  1. Direct: pi process with PPid=1 (pi itself is a direct child of init)
//  2. Wrapped: pi's parent is a bash process with PPid=1 (docker exec
//     disconnect: bash wrapper gets reparented to init, pi stays child of bash)
func scanOnce(pidName string) ([]int, error) {
	return scanOnceFromDir("/proc", pidName)
}
...
// scanOnceFromDir scans a /proc-like directory for orphaned processes
// matching pidName. Detects two orphan patterns:
//  1. Direct: process with PPid=1 and cmdline matches pidName
//  2. Wrapped: process whose parent (a bash/sh wrapper) has PPid=1
//     and whose cmdline matches pidName
func scanOnceFromDir(procDir, pidName string) ([]int, error) {
	...
	// First pass: collect all processes with PPid=1 (orphan wrappers)
	orphanParents := make(map[int]bool) // set of PIDs whose PPid=1
	for _, e := range entries {
		...
		if ppid == 1 {
			orphanParents[pid] = true
		}
	}

	// Second pass: find matching processes that are direct orphans
	// (PPid=1) or children of an orphan parent (bash wrapper with PPid=1)
	var orphans []int
	for _, e := range entries {
		if !e.IsDir() {
			continue
		...
		// Check for direct orphan (PPid=1) or wrapped orphan (parent has PPid=1)
		isOrphan := ppid == 1 || orphanParents[ppid]
		if !isOrphan {
			continue
		}
