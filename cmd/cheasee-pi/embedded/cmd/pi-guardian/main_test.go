...
func TestEscalate_SIGKILLAfterGrace(t *testing.T) {
	...
	// Reap the zombie to clear process table before checking
	// After SIGKILL, the process is a zombie until Wait() is called.
	// syscall.Kill(pid, 0) returns nil for zombies (process entry still exists),
	// so we must Wait() to reap before verifying the process is gone.
	proc.Wait()

	// Verify process is dead by checking /proc/<pid>/stat state
	data, err := os.ReadFile(filepath.Join("/proc", strconv.Itoa(proc.Pid), "stat"))
	if err == nil {
		// Parse state field (3rd field after the closing paren)
		s := string(data)
		closeParen := strings.LastIndex(s, ")")
		if closeParen >= 0 {
			fields := strings.Fields(s[closeParen+1:])
			if len(fields) > 0 && fields[0] == "Z" {
				t.Errorf("process %d is still a zombie after escalate+Wait", proc.Pid)
			}
		}
	} else {
		// Process doesn't exist in /proc at all — completely gone, which is fine
	}

	if !strings.Contains(buf.String(), "SIGKILL") {
		t.Errorf("SIGKILL log missing, got: %s", string(buf.Bytes()))
	}
}
