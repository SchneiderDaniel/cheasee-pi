...
func runInitDockerCheck(ctx context.Context) error {
	...
	if !result.Installed {
		return fmt.Errorf("Docker is not installed — cheasee-pi runs the pi coding agent inside a Docker container, so Docker Engine 24.0+ is required. Install it from https://docs.docker.com/engine/install/, then run `cheasee-pi init` again.")
	}
	if !result.Running {
