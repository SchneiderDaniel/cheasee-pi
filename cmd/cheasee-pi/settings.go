...
// memoryLimitEnv reads docker.memory from .pi/settings.json and returns the
// CHEASEEPI_MEMORY env entry to apply to the docker compose command. Missing
// file or empty memory → ("", false) with no output, preserving today's
// silent skip; corrupt JSON → warning on stderr and ("", false) — the limit
// is advisory, never fatal.
func memoryLimitEnv(workdir string) (string, bool) {
	settings, err := LoadSettings(workdir)
	if err != nil {
		if !os.IsNotExist(err) {
			fmt.Fprintf(os.Stderr, "  ⚠ Could not read .pi/settings.json: %v\n", err)
		}
		return "", false
	}
...
// .pi/settings.json to a docker compose command and announces it. No-op when
